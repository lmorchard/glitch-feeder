...
function find(base_url, body, real_next) {
  console.log("THUMB EXTRACTOR START FIND");
  ...
  console.log("THUMB EXTRACTOR START CHEERIO PARSE");
  var $;
  try {
    $ = cheerio.load(body);
  } catch (e) {
  ...
  console.log("THUMB EXTRACTOR END CHEERIO PARSE");
  ...
  console.log("THUMB EXTRACTOR NO OG");
  ...
  console.log("THUMB EXTRACTOR NO TWITTER");
  ...
  console.log("THUMB EXTRACTOR NO OLD FB");
  ...
  console.log("THUMB EXTRACTOR START LARGE IMG");
  for (var i = 0; i < containers.length; i++) {    
    // Assemble the selector, gather images.
    var sel = containers[i];
    console.log("THUMB EXTRACTOR IMG SEARCH", sel);
    var imgs = $(sel ? sel + " img" : "img");
    console.log("THUMB EXTRACTOR IMG SEARCH", sel, imgs.length);
    if (!imgs.length) {
      continue;
    }
  ...
  console.log("THUMB EXTRACTOR NOT FOUND");
...
function fetch(url, next) {
  ...
  console.log("THUMB EXTRACTOR START REQ");
  request(req_opts, function(err, req, body) {
    console.log("THUMB EXTRACTOR END REQ");
    if (err) {
      return find(url, "", next);
    }
    ...
    console.log("THUMB EXTRACTOR CONTENT", contentType);
    ...
    if (
      ...
      console.log("THUMB EXTRACTOR DE-GZIP");
...
module.exports = {
  _cache: _cache,
  find: find,
  // HACK: this should all be reworked to use promises
  fetch: util.promisify(fetch),
};
